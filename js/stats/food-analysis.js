...
import { isDayAllowedByStatsFilter } from './charts.js';
 ...
 */
export async function updateFoodAnalysis(period, foods = {}, composedMeals = {}) {
    currentFoodPeriod = period;
    
    // Collecter les données sur la période
    const foodConsumption = await collectFoodConsumption(period, foods, composedMeals);
 ...
 */
async function collectFoodConsumption(period, foods, composedMeals = {}) {
    const consumption = {};
    const today = await db.getStatsReferenceEndDate();
    ...
    for (let i = period - 1; i >= 0; i--) {
        ...
        const totals = utils.calculateDayTotals(meals, foods, composedMeals);
        if (!isDayAllowedByStatsFilter({ date: utils.formatDateKey(date), ...totals })) continue;
        
        // Parcourir tous les repas
        for (const mealType in meals) {
            if (Array.isArray(meals[mealType])) {
                meals[mealType].forEach(item => {
                    // Les repas composés vivent dans le store `meals` et non `foods` :
                    // sans ce test, chaque ligne « repas » disparaissait de l'analyse.
                    const itemId = utils.getMealItemId(item);
                    const food = item.isMeal && composedMeals[itemId] ? composedMeals[itemId] : foods[itemId];
                    if (food) {
                        const itemNutrition = utils.calculateMealItemNutrition(item, foods, composedMeals);
                        const isComposedMeal = Boolean(item.isMeal && composedMeals[itemId]);
                        if (!consumption[itemId]) {
                            consumption[itemId] = {
                                ...
                                isMeal: isComposedMeal
                            };
                        }

                        // Poids consommé : pour un repas composé non ajustable, le
                        // poids réellement consommé est celui de la ligne.
                        const consumedWeight = Number(item.weight) || 0;

                        // Ajouter le poids
                        consumption[itemId].totalWeight += consumedWeight;

                        // Coût (gère portions personnalisées, repas ajustables et prix custom)
                        consumption[itemId].totalCost += utils.calculateMealItemCost(item, foods, composedMeals);

                        // Macros : on utilise la source de vérité partagée
                        consumption[itemId].totalProteins += itemNutrition.proteins;
                        consumption[itemId].totalCarbs += itemNutrition.carbs;
                        consumption[itemId].totalFats += itemNutrition.fats;
                        consumption[itemId].totalFibers += itemNutrition.fibers;
                        consumption[itemId].totalSugars += itemNutrition.sugars;
                    }
                });
            }
        }
    ...
    Object.values(consumption).forEach(item => {
        const food = item.isMeal && composedMeals[item.id] ? composedMeals[item.id] : foods[item.id];
        if (food && hasPrice(food) && item.proteinsPer100g > 0) {
            ...
            item.proteinPrice = pricePer100g !== null ? (pricePer100g / item.proteinsPer100g) * 100 : 0;
        } else {
            item.proteinPrice = 0;
        }
    });
...
function updateFoodAnalysisTable(foodData) {
    ...
    sortedData.forEach(food => {
        ...
        // `food.name` peut venir d'un import JSON : on construit les cellules avec
        // textContent pour éviter toute injection HTML (XSS stocké).
        const cells = [
            food.name,
            `${(food.totalWeight / 1000).toFixed(2)} kg`,
            `${food.totalCost.toFixed(2)} €`,
            `${food.totalProteins.toFixed(1)} g`,
            `${food.totalCarbs.toFixed(1)} g`,
            `${food.totalFats.toFixed(1)} g`,
            `${food.totalFibers.toFixed(1)} g`,
            `${food.totalSugars.toFixed(1)} g`,
            `${food.proteinsPer100g.toFixed(1)} g`,
            food.proteinPrice > 0 ? `${food.proteinPrice.toFixed(2)} €` : 'N/A',
        ];
        const columns = ['name', 'weight', 'cost', 'proteins', 'carbs', 'fats', 'fibers', 'sugars', 'proteinsPer100g', 'proteinPrice'];
        cells.forEach((value, index) => {
            const td = document.createElement('td');
            td.textContent = String(value);
            if (index > 0) td.classList.add('macro-column');
            td.dataset.column = columns[index];
            if (columns[index] === 'name') td.removeAttribute('data-column');
            row.appendChild(td);
        });
...
function showCopyNotification(message) {
    ...
    notification.style.cssText = `
        ...
        z-index: 10000;
        font-weight: 600;
        animation: analyticsToastIn 0.3s ease;
    `;
    
    document.body.appendChild(notification);
    
    setTimeout(() => {
        notification.style.animation = 'analyticsToastOut 0.3s ease';
