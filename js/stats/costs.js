...
function updateCostSummaryCards(data, period) {
    ...
    document.getElementById('avgCostPeriod').textContent = `${data.length} jour${data.length > 1 ? 's' : ''} comptabilisé${data.length > 1 ? 's' : ''}`;
        ...
        for (const mealType in meals) {
            if (Array.isArray(meals[mealType])) {
                meals[mealType].forEach(item => {
                    ...
                    if (food) {
                        // Source de vérité unique : `calculateMealItemCost` gère les
                        // portions personnalisées ET `customPrice`, qui était ignoré ici
                        // (le Top 5 divergeait des cartes et du graphe quotidien).
                        const itemCost = utils.calculateMealItemCost(item, foods, composedMeals);
...
async function createCostComparisonChart(foods, composedMeals = {}) {
    ...
    for (const period of periods) {
        ...
        costs.push((includedDays > 0 ? totalCost / includedDays : 0).toFixed(2));
    }
