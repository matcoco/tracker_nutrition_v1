 ...
 */
export async function updateActivityCharts(period, foods = {}, composedMeals = {}) {
    ...
    for (let i = period - 1; i >= 0; i--) {
        ...
        if (!isDayAllowedByStatsFilter({ date: dateKey, ...totals })) continue;
        ...
        activities.forEach(activity => {
            // Coercition explicite : une activité importée avec des valeurs
            // textuelles produisait une concaténation ("0300200") ou un NaN.
            const activityCalories = Number(activity.calories) || 0;
            const activityDuration = Number(activity.duration) || 0;
            dailyCalories += activityCalories;
            dailyDuration += activityDuration;
            ...
            typeStats[activity.type].duration += activityDuration;
            typeStats[activity.type].calories += activityCalories;
        });
    ...
    createWeeklyComparisonChart(foods, composedMeals);
}
 ...
 */
async function createWeeklyComparisonChart(foods = {}, composedMeals = {}) {
    const periods = [7, 30];
    const avgCalories = [];
    const avgDuration = [];
    ...
    for (const period of periods) {
        ...
        avgCalories.push((includedDays > 0 ? totalCalories / includedDays : 0).toFixed(0));
        avgDuration.push((includedDays > 0 ? totalDuration / includedDays : 0).toFixed(0));
    }
