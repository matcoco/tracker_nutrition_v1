...
export function calculateMealNutrition(ingredients, foods) {
    // Un appel avec des ingrédients absents levait un TypeError.
    ...
    ingredients.forEach(ingredient => {
        ...
        const factor = (Number(ingredient.weight) || 0) / 100;
...
function setupEventListeners(foods) {
    ...
    // ... et quand on bascule « Portions ajustables » (l'aperçu divergeait alors
    // des valeurs réellement enregistrées).
    const adjustableCheckbox = document.getElementById('mealIsPortionAdjustable');
    if (adjustableCheckbox && !adjustableCheckbox.dataset.previewBound) {
        adjustableCheckbox.dataset.previewBound = 'true';
        adjustableCheckbox.addEventListener('change', () => updateMealPreview(foods));
    }
...
function updateTotalWeightFromIngredients() {
    ...
    // Ne pas écraser une valeur saisie à la main (poids cuit, perte d'eau…) :
    // on ne remplace que si le champ est vide ou contient encore la valeur
    // issue du dernier calcul automatique.
    const totalWeightInput = document.getElementById('mealTotalWeight');
    if (!totalWeightInput) return;
    const previousAuto = Number(totalWeightInput.dataset.autoWeight || '');
    const isUntouched = !totalWeightInput.value
        || (Number.isFinite(previousAuto) && Number(totalWeightInput.value) === previousAuto);
    if (calculatedWeight > 0 && isUntouched) {
        totalWeightInput.value = calculatedWeight;
        totalWeightInput.dataset.autoWeight = String(calculatedWeight);
    }
}
...
function updateMealPreview(foods) {
    ...
    // Un repas ajustable enregistre les valeurs TOTALES de la recette : l'aperçu
    // doit refléter ce qui sera réellement sauvegardé, et non une conversion
    // systématique en /100 g (l'écart pouvait atteindre un facteur 3 à 5).
    const isAdjustable = document.getElementById('mealIsPortionAdjustable')?.checked === true;

    let displayLabel = 'Totaux nutritionnels';
    if (isAdjustable) {
        displayLabel = `Valeurs TOTALES de la recette${totalWeight > 0 ? ` (${totalWeight}g)` : ''}`;
