...
import { getAllFromStore } from '../core/db.js';
import { calculateDayTotals, formatDateKey } from '../core/utils.js';
import { isDayAllowedByStatsFilter } from '../stats/charts.js';
...
async function _pushDay(date, days, foods, composedMeals) {
    ...
    if (!isDayAllowedByStatsFilter({ date: formatDateKey(date), ...totals })) return;
...
function buildMealSections(meals, foods, composedMeals) {
    ...
    return sections.join('') || '<p class="journal-empty">Aucun aliment enregistré.</p>';
}
